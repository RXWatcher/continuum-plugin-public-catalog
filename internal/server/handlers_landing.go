...
// runs in standalone-listener mode with a SPECIFIC host (not wildcard),
// synthesises the URL from the listener address.
//
// When the listener is wildcard (`:port` / `0.0.0.0`) and PublicBaseURL
// is unset, returns "" rather than guessing — the request's Host header
// might be the admin-host, the public-host, or a load-balancer DNS name,
// and emitting a wrong absolute URL into a bypass-link is worse than
// returning a plugin-relative one (the SPA's absoluteURL() then resolves
// it against window.location.origin, which is at least predictable).
// Generated bypass links won't carry a public hostname until the
// operator sets `public_base_url`.
func publicBaseURLForRequest(r *http.Request, d Deps) string {
	...
	if host == "" || host == "0.0.0.0" || host == "::" || host == "[::]" {
		// Wildcard listen with no PublicBaseURL: don't synthesise a
		// guess. Frontend falls back to window.location.origin.
		return ""
	}
